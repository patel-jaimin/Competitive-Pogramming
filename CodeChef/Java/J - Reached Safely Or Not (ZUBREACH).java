...
class Codechef
{
	public static void main (String[] args) throws java.lang.Exception
	{
		 ...
		 while(t!=0){
		     ...
		     int D2=sc.nextInt();
		     move=sc.nextInt();
		     String moveSequence=sc.next();
		     for(int i=0;i<move;i++)
		     {
		         char direction=moveSequence.charAt(i);
		         switch(direction){
		             case 'U':
		                 Ry+=1;
		                 break;
		       ...
		       if(Rx>m||Ry>n||Rx<0||Ry<0)
		        System.out.println("Case "+count+": DANGER");
		       else if(Rx==D1&&Ry==D2)
		        System.out.println("Case "+count+": REACHED");
		        else 
		           System.out.println("Case "+count+": SOMEWHERE");
		      count++;     
		      t--;
		 }
