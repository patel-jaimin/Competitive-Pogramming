...
# Any conflicting locks are taken care by wait-die wait mechanism to avoid deadlocks.
        ...
        if length != 0:
            for i in range(0, length):
                if lockTableObjects[i].lockedDateItem == resourceName:
                    flag = 1
                    if lockTableObjects[
                        i].lockState == WL:  # This checks if the resource that's been requested in under a writeLock by any other transaction and if True, wait die is called
                        ...
                        waitDie(searchTransactionID(transactionID),
...
# It checks for the transaction state before performing the operation.
# Any conflicting locks are taken care by  wait die mechanism to avoid deadlocks.
                    ...
                    if lockTableObjects[i].lockState == RL:
                        if len(lockTableObjects[i].lockHeldBy) == 1: 
                            if lockTableObjects[i].lockHeldBy[
                                ...
                                print("calling wait-die mechanism")  # call wait die if its under readlock by another transaction
                                waitDie(searchTransactionID(transactionID),
...
# the  wait die deadlock prevention mechanism
def waitDie(requestingTransaction, holdingTransaction, lockedResource, operation):
    if requestingTransaction.timestamp > holdingTransaction.timestamp:
        ...
        unlock(
            requestingTransaction.transactionID)  # unlocking all the resources of the transaction that was aborted by wait-die
    else:
...
def checkDuplicateTransaction(requestingTransaction):
    for t in waitingTransactions:
        if requestingTransaction.timestamp > t.timestamp :# if the witing transcation timesstamp is more then the requesting timestamp. Waiting one aborts
            requestingTransaction.changeTransactionState(A)
